...
真正的Montgomery实现不能只在最后做 `to_mont/from_mont`，而要让乘法本身发生在Montgomery域中：
...
因此本实现做了三件事：
- `pow36[]` 预先转换为Montgomery域中的 `pow36_mont[]`
- 每个输入字符值通过 `to_mont()` 转入Montgomery域
- 乘法使用 `montgomery_mul()`，累加保持在Montgomery域，最后再 `from_mont()` 转回普通整数

### 5.3 性能分析

实测性能约**193.17ns/次**，结果正确，但明显慢于纯SIMD版本。

**原因分析：**
- 普通SIMD版本的核心乘法是 `_mm_mullo_epi32`，一次并行处理4组乘法
- 当前Montgomery版本是真实Montgomery域计算，但每个字符都要做 `to_mont + montgomery_mul`，退化为30轮标量计算
- MOD 97校验只涉及很少的模乘，Montgomery的转换成本无法被摊薄
- Montgomery模乘更适合RSA、ECC、模幂这类存在大量连续模乘的场景
...
| 实现方案 | 单次耗时 | vs Python `int(s,36)` | vs Python Horner |
|---------|---------|------------------------|------------------|
| **C++ LUT SIMD FixedPrefix** | **17.87 ns** | **18.77x** 🔥 | **350.6x** 🔥 |
| C++ LUT SIMD | 30.14 ns | 11.13x 🔥 | 207.9x 🔥 |
| C++ LookupTable | 67.46 ns | 4.97x | 92.9x |
| C++ Horner | 97.65 ns | 3.43x | 64.2x |
| **Python `int(s, 36)` (C实现)** | **335 ns** | **1.00x** | 18.7x |
| C++ Montgomery | 193.17 ns | 1.73x | 32.4x |
| Python Horner | 6265 ns | 0.05x ❌ | 1.00x |

### 7.3 C++内部对比（Horner = 1.00x）

| 实现方案 | 单次耗时 | 相对性能 | vs Horner |
|---------|---------|---------|----------|
| **LUT SIMD FixedPrefix** | **17.87 ns** | **5.46x** | ✅ +446% |
| LUT SIMD | 30.14 ns | 3.24x | ✅ +224% |
| LookupTable | 67.46 ns | 1.45x | ✅ +45% |
| Horner (基准) | 97.65 ns | 1.00x | 基线 |
| Montgomery | 193.17 ns | 0.51x | ❌ -49% |

### 7.4 结果解读

1. **Python循环是最大瓶颈**：纯Python Horner比C实现的`int(s, 36)`慢 **18.7倍**
2. **C++编译优化带来质变**：C++ Horner比Python内置C函数再快 **3.4倍**
3. **SIMD收益显著**：纯SIMD相比Horner获得 **3.2倍**加速
4. **业务场景优化价值巨大**：固定前缀编译期优化让SIMD再快 **1.7倍**，相比Horner达到 **5.5倍**
5. **Montgomery真实实现但不适合本场景**：由于退化为标量Montgomery乘法，只有 **0.51x** Horner性能
6. **完整优化路径总加速比**：Python循环 → Python内置C → C++标量 → C++ SIMD → 业务常量折叠 = **350倍**性能提升！
...
4. **SIMD并行计算**获得了3.2倍的实际加速，是通用场景下的最优解
5. **业务场景优化价值巨大**：利用固定前缀的业务特性，SIMD FixedPrefix版本达到了相比Horner **5.5倍**的惊人性能
6. **Montgomery模乘**已经真实实现，但在当前短ID、少量模乘场景下收益为负，仅在多步模运算场景中值得考虑

**完整优化路径总收益：纯Python → Python内置C → C++标量 → C++ SIMD → 业务常量折叠 = **350倍**性能提升！**
...
**编译运行：**
```bash
mkdir -p ../../.build && g++ -O3 -msse4.1 benchmark.cpp -o ../../.build/benchmark && python3 benchmark.py
```

**仅运行C++ Benchmark：**
```bash
mkdir -p ../../.build && g++ -O3 -msse4.1 benchmark.cpp -o ../../.build/benchmark && ../../.build/benchmark
```
