...
public:
    vector<int> maximumCount(vector<int>& nums, vector<vector<int>>& queries) {
        // 参考题解 - 这题的思维难点在于如何统计最大值
        // 根据题解代码实现，是当前区间 + 1, 外围区间取不同区间的最大值
        int n = nums.size();
        ...
        // 下推懒标记
        auto pushdown = [&] (int s, int t, int p) -> void{
    ...
    vector<int> maximumCount2(vector<int>& nums, vector<vector<int>>& queries) {
        // 参考题解 - 代码优化
        int n = nums.size();
        // 参考 oi-wiki 线段树模板 - 区间更新 最大值
        vector<int> tree(4 * n);
        ...
        unordered_map<int, set<int>> mp;
        // 区间 收益
        auto apply = [&](int key, int val) -> void {
            if (mp.count(key) && mp[key].size()) {
                int l = *mp[key].begin(), r = *mp[key].rbegin();
                if (l!=r) update(l, r, val, 0, n-1, 0);
            }
        };
        for (int i = 0; i < n; ++i) if (is_prime[nums[i]]) mp[nums[i]].insert(i);
        for (auto& [x, _]: mp) apply(x, 1);
        ...
        for (auto& q : queries) {
            ...
            if (nums[inx] != val) {
                if (is_prime[nums[inx]]) {
                    apply(nums[inx], -1);             // 撤销原有区间收益
                    mp[nums[inx]].erase(inx); 
                    if (mp[nums[inx]].size()==0) mp.erase(nums[inx]);
                    apply(nums[inx], 1);              // 增加变更后区间收益
                }
                nums[inx] = val;
                if (is_prime[val]) {
                    apply(val, -1);             // 撤销原有区间收益
                    mp[nums[inx]].insert(inx);
                    apply(val, 1);              // 增加变更后区间收益
                }
            }
            ans.push_back(tree[0]+mp.size());
        }
