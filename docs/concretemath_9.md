...
$$
f(0)=\int_0^1 f(x)dx-\frac12\bigl(f(1)-f(0)\bigr) +\int_0^1\Bigl(x-\frac12\Bigr)f'(x)dx.
$$

（伯努利多项式 $B_m(x)$ 一般定义为
$$
B_m(x) = \binom{m}{0}B_0x^m + \binom{m}{1}B_1x^{m-1} + \cdots + \binom{m}{m}B_mx^0,
\tag{9.72}
$$
特别地，$B_1(x) = x - \frac12$。）换言之，我们要证明的是
$$
\frac{f(0)+f(1)}{2}
= \int_0^1 f(x)\,dx + \int_0^1 \Bigl(x-\frac12\Bigr)f'(x)\,dx.
$$
但这只是分部积分公式
$$
u(x)v(x)\bigg|_0^1
= \int_0^1 u(x)\,dv(x) + \int_0^1 v(x)\,du(x)
\tag{9.73}
$$

取 $u(x)=f(x)$、$v(x)=x-\frac12$ 的特殊情形。因此 $m=1$ 的情形很容易证明。

当 $m>1$ 时，要从 $m-1$ 递推到 $m$ 并完成归纳，只需证明 $R_{m-1} = \frac{B_m}{m!}f^{(m-1)}(x)\bigg|_0^1 + R_m$ 即
$$
\begin{aligned}
(-1)^m &\int_0^1 \frac{B_{m-1}(x)}{(m-1)!}f^{(m-1)}(x)\,dx \\
&= \frac{B_m}{m!}f^{(m-1)}(x)\bigg|_0^1- (-1)^m \int_0^1 \frac{B_m(x)}{m!}f^{(m)}(x)\,dx.
\end{aligned}
$$
上式可化简为
$$
(-1)^m B_m f^{(m-1)}(x)\bigg|_0^1= m\int_0^1 B_{m-1}(x)f^{(m-1)}(x)\,dx + \int_0^1 B_m(x)f^{(m)}(x)\,dx.
$$

再次对这两个积分使用 (9.73)，取 $u(x)=f^{(m-1)}(x)$、$v(x)=B_m(x)$，因为伯努利多项式 (9.72) 的导数为
$$
\begin{aligned}
\frac{d}{dx}\sum_k \binom{m}{k}B_kx^{m-k} &= \sum_k \binom{m}{k}(m-k)B_kx^{m-k-1} \\
&= m\sum_k \binom{m-1}{k}B_kx^{m-1-k} = mB_{m-1}(x).
\end{aligned}
\tag{9.74}
$$
（这里用到了吸收恒等式 (5.7)。）因此所需公式成立当且仅当
$$
(-1)^m B_m f^{(m-1)}(x)\bigg|_0^1
= B_m(x)f^{(m-1)}(x)\bigg|_0^1.
$$

换言之，我们需要满足
$$
(-1)^m B_m = B_m(1) = B_m(0),\quad m>1. \tag{9.75}
$$
这看起来有点尴尬，因为 $B_m(0)$ 显然等于 $B_m$，而不是 $(-1)^m B_m$。但实际上并没有问题，因为 $m>1$：我们知道当 $m$ 为奇数时 $B_m=0$。（尽管如此，还是差一点出错。）

要完成欧拉求和公式的证明，只需证明 $B_m(1)=B_m(0)$，这等价于
$$
\sum_k \binom{m}{k}B_k = B_m,\quad m>1.
$$
而这正是伯努利数的定义式 (6.79)，于是证明完毕。

恒等式 $B_m'(x)=mB_{m-1}(x)$ 蕴含
$$
\int_0^1 B_m(x)\,dx = \frac{B_{m+1}(1)-B_{m+1}(0)}{m+1},
$$
我们现在知道当 $m\ge1$ 时该积分为 0。因此欧拉公式中的余项
$$
R_m = \frac{(-1)^{m+1}}{m!}\int_a^b B_m\bigl(\{x\}\bigr)f^{(m)}(x)\,dx
$$
是用平均值为 0 的函数 $B_m(\{x\})$ 去乘 $f^{(m)}(x)$。这意味着 $R_m$ 很有可能很小。

我们更仔细地考察 $0\le x\le1$ 上的 $B_m(x)$，因为它决定了余项 $R_m$ 的行为。下面是前 12 个 $m$ 对应的 $B_m(x)$ 图像：

虽然 $B_3(x)$ 到 $B_9(x)$ 都相当小，但伯努利多项式与数最终会变得很大。幸运的是，$R_m$ 中有补偿因子 $1/m!$，起到了抑制作用。

当 $m\ge3$ 时，$B_m(x)$ 的图像开始非常接近正弦曲线；习题 58 证明，$B_m(x)$ 事实上可以被 $-\cos(2\pi x-\frac12\pi m)$ 的某个倍数很好地逼近，相对误差为 $1/2^m$。

一般地，$B_{4k+1}(x)$ 在 $0<x<\frac12$ 时为负，在 $\frac12<x<1$ 时为正。因此它的积分 $B_{4k+2}(x)/(4k+2)$ 在 $0<x<\frac12$ 递减，在 $\frac12<x<1$ 递增。此外有
$$
B_{4k+1}(1-x) = -B_{4k+1}(x),\quad 0\le x\le 1,
$$
由此可得
$$
B_{4k+2}(1-x) = B_{4k+2}(x),\quad 0\le x\le 1.
$$

常数项 $B_{4k+2}$ 使得积分 $\int_0^1 B_{4k+2}(x)dx=0$，因此 $B_{4k+2}>0$。$B_{4k+2}(x)$ 的积分是 $B_{4k+3}(x)/(4k+3)$，故它在 $0<x<\frac12$ 为正，$\frac12<x<1$ 为负；且
$$
B_{4k+3}(1-x) = -B_{4k+3}(x),
$$
所以 $B_{4k+3}(x)$ 具有与 $B_{4k+1}(x)$ 相反的性质。于是 $B_{4k+4}(x)$ 与 $B_{4k+2}(x)$ 性质相反，$B_{4k+5}(x)$ 与 $B_{4k+1}(x)$ 相同；我们完成了一个循环，归纳地对所有 $k$ 证明了上述性质。

根据这一分析，$B_{2m}(x)$ 的最大值必出现在 $x=0$ 或 $x=\frac12$。习题 17 证明
$$
B_{2m}\Bigl(\frac12\Bigr) = (2^{1-2m}-1)B_{2m}, \tag{9.76}
$$
因此
$$
\bigl|B_{2m}(\{x\})\bigr| \le |B_{2m}|. \tag{9.77}
$$

这可用来给出欧拉求和公式余项的一个有用上界，因为由 (6.89) 知
$$
\frac{|B_{2m}|}{(2m)!}
= \frac{2}{(2\pi)^{2m}}\sum_{k\ge1}\frac{1}{k^{2m}}
= O\bigl((2\pi)^{-2m}\bigr),\quad m>0.
$$
...
\begin{aligned}
\sum_{a\le k<b} f(k)
= \int_a^b f(x)dx & - \frac12 f(x)\bigg|_a^b + \sum_{k=1}^m \frac{B_{2k}}{(2k)!}f^{(2k-1)}(x)\bigg|_a^b  \\
&+ O\bigl((2\pi)^{-2m}\bigr)\int_a^b\bigl|f^{(2m)}(x)\bigr|dx.
\end{aligned}
\tag{9.78}
$$

例如，若 $f(x)=e^x$，所有导数都相同，公式给出 $ \sum_{a\le k<b}e^k= (e^b-e^a)\Bigl(1-\frac12+\frac{B_2}{2!}+\cdots+\frac{B_{2m}}{(2m)!}\Bigr)+O\bigl((2\pi)^{-2m}\bigr)$。 当然我们知道该和其实是等比级数，等于 $(e^b-e^a)/(e-1)
=(e^b-e^a)\sum_{k\ge0}B_k/k!$。

若在 $a\le x\le b$ 上 $f^{(2m)}(x)\ge0$，则积分 $\int_a^b|f^{(2m)}(x)|dx$ 就是 $f^{(2m-1)}(x)\big|_a^b$，于是
$$
|R_{2m}|
\le \left| \frac{B_{2m}}{(2m)!}f^{(2m-1)}(x)\bigg|_a^b \right|;
$$

换言之，此时余项被最后一项（余项前那一项）的模控制。若进一步知道
$$
f^{(2m+2)}(x)\ge0,\quad f^{(2m+4)}(x)\ge0,\quad a\le x\le b, \tag{9.79}
$$

则可以得到更好的估计：
$$
R_{2m}
= \theta_m \frac{B_{2m+2}}{(2m+2)!}f^{(2m+1)}(x)\bigg|_a^b,\quad
\text{对某个 }0<\theta_m<1; \tag{9.80}
$$
即余项介于 0 与 (9.78) 中第一个被舍弃项之间。

证明如下：欧拉求和公式对任意 $m$ 成立，且 $m>0$ 时 $B_{2m+1}=0$，故 $R_{2m}=R_{2m+1}$，第一个被舍弃项为
$$R_{2m}-R_{2m+2}$$
只需证明 $R_{2m}$ 介于 0 与 $R_{2m}-R_{2m+2}$ 之间，即 $R_{2m}$ 与 $R_{2m+2}$ 符号相反。我们断言：
$$
a\le x\le b \text{ 上 } f^{(2m+2)}(x)\ge0
\;\Longrightarrow\; (-1)^m R_{2m}\ge0. \tag{9.81}
$$
结合 (9.79) 即得 $R_{2m}$ 与 $R_{2m+2}$ 异号，从而 (9.80) 得证。

(9.81) 不难证明，只需回忆 $R_{2m+1}$ 的定义以及 $B_{2m+1}(x)$ 的图像性质：
$$
R_{2m}=R_{2m+1}
= \int_a^b \frac{B_{2m+1}(\{x\})}{(2m+1)!}f^{(2m+1)}(x)dx,
$$
...
在卸下“辅助轮”之前，我们从更高视角回顾刚才的过程。我们从和式
$$
S_n = \sum_{1\le k<n} f(k)
$$
出发，用欧拉求和公式写成
$$
S_n = F(n)-F(1) + \sum_{k=1}^m\bigl(T_k(n)-T_k(1)\bigr) + R_m(n), \tag{9.83}
$$

其中 $F(x)=\int f(x)dx$，$T_k(x)$ 是包含 $B_k$ 与 $f^{(k-1)}(x)$ 的项。我们还发现存在常数 $c$，使得
$$
f^{(m)}(x) = O(x^{c-m}),\quad\text{ as } x\to\infty,\quad \text{ for all large }m.
$$

（本例中 $f(x)=1/x(x+1)$，$F(x)=\ln\bigl(x/(x+1)\bigr)$，$c=-2$，$T_k(x)=(-1)^{k+1}(B_k/k)\bigl(x^{-k}-(x+1)^{-k}\bigr)$。）对充分大的 $m$，这意味着余项的尾部很小：
$$
\begin{aligned}
R_m'(n) &= R_m(\infty)-R_m(n) \\
&= (-1)^{m+1}\int_n^\infty \frac{B_m(\{x\})}{m!}f^{(m)}(x)dx = O(n^{c+1-m}). \tag{9.84}
\end{aligned}
$$

因此可以推出：存在常数 $C$，使得
$$
S_n = F(n)+C + \sum_{k=1}^m T_k(n) - R_m'(n). \tag{9.85}
$$
（常数 $C$ 很好地吸收了麻烦的 $T_k(1)$ 项。）

今后只要 $R_m(\infty)$ 存在，我们就可以直接断言 $C$ 存在，从而省去多余计算。

现在假设在 $1\le x\le n$ 上 $f^{(2m+2)}(x)\ge0$ 以及 $\quad f^{(2m+4)}(x)\ge0$。我们已证明这会给出余项的简单界 (9.80)：
$$
R_{2m}(n) = \theta_{m,n}\bigl(T_{2m+2}(n)-T_{2m+2}(1)\bigr),
\quad 0<\theta_{m,n}<1.
$$

但我们并不想要包含 $R_{2m}(n)$ 与 $T_{2m+2}(1)$ 的界；毕竟引入 $C$ 就是为了消去 $T_k(1)$。我们真正想要的是形如以下的界。
$$
-R_{2m}'(n) = \phi_{m,n}\,T_{2m+2}(n)
$$

其中 $0<\phi_{m,n}<1$；这样就能从 (9.85) 推出
$$
S_n = F(n)+C+T_1(n)+\sum_{k=1}^m T_{2k}(n) +\phi_{m,n}T_{2m+2}(n), \tag{9.86}
$$
于是余项真正介于 0 与第一个被舍弃项之间。

对之前的论证略作修改就能完美解决。我们假设：
$$
f^{(2m+2)}(x)\ge0,\quad f^{(2m+4)}(x)\ge0,\quad x\to\infty. \tag{9.87}
$$

就余项部分而言，(9.85) 的右边与取 $a=n,b=\infty$ 的欧拉公式 (9.67) 的右边只差一个负号；而逐次余项是由对 $m$ 归纳得到的。因此之前的论证可以直接沿用。

**求和示例 2：调和数的调和渐近**
从这个简单（但稳妥）的例子学到这么多后，我们可以轻松处理一个非平凡问题：用**欧拉求和公式**推导我们一直提到的调和数 $H_n$ 渐近展开。

这里取 $f(x) = \frac1x$。积分与各阶导数我们在示例 1 中已经熟悉；且当 $x\to\infty$ 时 $f^{(m)}(x) = O(x^{-m-1})$。因此可直接代入公式 (9.85)：
$$
\sum_{1\le k<n}\frac1k = \ln n + C + B_1 n^{-1} - \sum_{k=1}^m \frac{B_{2k}}{2k\,n^{2k}} - R_{2m}'(n),
$$
其中 $C$ 是某个常数。左边是 $H_{n-1}$ 而非 $H_n$；先用 $H_{n-1}$ 计算，最后再加上 $1/n$ 更方便。此时 $B_1 n^{-1}$ 变为 $(B_1+1)n^{-1} = \frac1{2n}$。我们把常数 $C$ 改记为 $\gamma$，因为欧拉常数 $\gamma$ 正是定义为 $\lim_{n\to\infty}(H_n - \ln n)$。

余项可以用刚才的理论很好估计，因为对所有 $x>0$：$f^{(2m)}(x) = (2m)!/x^{2m+1} \ge 0$。
因此由 (9.86) 得
$$
H_n = \ln n + \gamma + \frac1{2n} - \sum_{k=1}^m \frac{B_{2k}}{2k\,n^{2k}} + \theta_{m,n}\,\frac{B_{2m+2}}{(2m+2)\,n^{2m+2}}, \tag{9.88}
$$

其中 $0 < \theta_{m,n} < 1$。这就是表 452 中列出前几项的一般公式。例如取 $m=2$：
$$
H_n = \ln n + \gamma + \frac1{2n} - \frac1{12n^2} + \frac1{120n^4} - \frac{\theta_{2,n}}{252n^6}. \tag{9.89}
$$

顺便一提，即使取 $n=2$，这个式子也能给出 $\gamma$ 的不错近似：
$$
\gamma = H_2 - \ln2 - \frac14 + \frac1{48} - \frac1{1920} + \varepsilon
= 0.577165\ldots + \varepsilon,
$$

其中 $0<\varepsilon<\dfrac1{16128}$。若取 $n=10^4$、$m=250$，可得到 $\gamma$ 的1271 位精确小数，开头如下 [205]：
$$
\gamma=0.57721\;56649\;01532\;86060\;65120\;90082\;40243\ldots. \tag{9.90}
$$

不过欧拉常数 $\gamma$ 也出现在其他公式中，能以更高效率计算 [345]。

**求和示例 3：斯特林近似**
若 $f(x) = \ln x$，我们有 $f'(x) = 1/x$，因此可以用几乎与求和倒数相同的计算来求对数的和。欧拉求和公式给出
$$
\begin{aligned}
\sum_{1\le k<n}\ln k &= n\ln n - n + \sigma - \frac{\ln n}{2} \\
&+ \sum_{k=1}^m \frac{B_{2k}}{2k(2k-1)n^{2k-1}} + \phi_{m,n}\frac{B_{2+2}}{(2m+2)(2m+1)n^{2m+1}}
\end{aligned}
$$
其中 $\sigma$ 是某个常数，即**斯特林常数**，且 $0 < \phi_{m,n} < 1$。（在这种情况下 $f^{(2m)}(x)$ 是负数而非正数；但我们仍然可以说余项由第一个舍弃项控制，因为我们本可以从 $f(x) = -\ln x$ 而非 $f(x) = \ln x$ 开始。）两边同时加上 $\ln n$ 得到
$$
\ln n! = n\ln n - n + \frac{\ln n}{2} + \sigma + \frac{1}{12n} - \frac{1}{360n^3} + \frac{\phi_{2,n}}{1260n^5} \tag{9.91}
$$
对应 $m=2$。我们对两边取 $\exp$ 即可得到表 452 中的近似式。（结果表明 $e^\sigma = \sqrt{2\pi}$，但我们暂时还不准备推导这个公式。事实上，斯特林直到棣莫弗 [76] 证明该常数存在数年之后，才发现 $\sigma$ 的闭形式。）

若固定 $m$ 并令 $n\to\infty$，该一般公式对 $\ln n!$ 的绝对误差会越来越好，因此对 $n!$ 的相对误差也越来越好。但如果固定 $n$ 而增大 $m$，误差界 $|B_{2m+2}|/(2m+2)(2m+1)n^{2m+1}$ 会先减小到某个值然后开始增大。因此该近似存在一个临界点，超过该点后，某种不确定性原理会限制 $n!$ 可被近似的精度。

在第 5 章的式 (5.83) 中，我们利用欧拉提出的定义
$$
\frac{1}{\alpha!} = \lim_{n\to\infty}\binom{n+\alpha}{n}n^{-\alpha}
$$
将阶乘推广到任意实数 $\alpha$。假设 $\alpha$ 很大，则
$$
\ln\alpha! = \lim_{n\to\infty}\Bigl(\alpha\ln n + \ln n! - \sum_{k=1}^n\ln(\alpha+k)\Bigr),
$$
可以对 $f(x) = \ln(x+\alpha)$ 使用欧拉求和公式来估计这个和：
$$
\begin{aligned}
\sum_{k=1}^n\ln(k+\alpha) &= F_m(\alpha,n) - F_m(\alpha,0) + R_{2m}(\alpha,n), \\
F_m(\alpha,x) &= (x+\alpha)\ln(x+\alpha) - x + \frac{\ln(x+\alpha)}{2} + \sum_{k=1}^m\frac{B_{2k}}{2k(2k-1)(x+\alpha)^{2k-1}}, \\
R_{2m}(\alpha,n) &= \int_0^n\frac{B_{2m}(\{x\})}{2m(x+\alpha)^{2m}}dx.
\end{aligned}
$$
（这里我们在 (9.67) 中取 $a=0$，$b=n$，然后在两边加上 $\ln(n+\alpha)-\ln\alpha$。）如果用 $\ln n!$ 的斯特林近似减去 $\sum_{k=1}^n\ln(k+\alpha)$ 的近似，再加上 $\alpha\ln n$ 并令 $n\to\infty$，可得
$$
\ln\alpha! = \alpha\ln\alpha - \alpha + \frac{\ln\alpha}{2} + \sigma + \sum_{k=1}^m\frac{B_{2k}}{2k(2k-1)\alpha^{2k-1}} - \int_0^\infty\frac{B_{2m}(\{x\})}{2m(x+\alpha)^{2m}}dx,
$$
