...
## 循环展开
循环展开是一种程序变换，通过增加每次迭代计算的元素的数量，减少循环的迭代次数。psum2 函数就是这样的例子 (代码参见 code/prefix_sum.c)。循环迭代能从两个方面改程序的性能。
- 首先，它减少了不直接有助于程序结果的操作的数量。
- 其次，它提供了一些方法，可以进一步变化代码，减少整个计算中关键路径上的操作数量。

_循环展开只能改进整数加法和乘法的性能_ 当展开次数 k 介于 1 和 3 之间时，CPE 等于 3.0/k。结果证明编译器是基于重关联变换 (reassociation transformation) 做优化的，重关联变换就是改变值合并的顺序。



## 提高并行性
程序是受运算单元的延迟限制的。执行加法和乘法的功能单元是完全流水线化的，意味着每个时钟周期开始一个新操作。代码不能利用这种能力，即使循环也不能，这是因为我们将累积值放在一个单独的变量 acc 中。


### 多个累积变量
对于一个可结合和可交换的合并运算来说，可以通过将一组合并运算分割成两个或更多的部分，并在最终合并结果来提高性能。例如，$P_n$ 表示元素 $a_0, a_1, \cdots, a_{n-1}$ 的乘积：
$$
P_n = \prod_{i=0}^{n-1} a_i
$$
还可以把它写成 $P_n = PE_n \times PO_n$， 其中：
$$
PE_n = \prod_{i=0}^{n/2-1} a_{2i}, PO_n = \prod_{i=0}^{n/2-1} a_{2i+1}, 
$$
它既使用了两次循环展开，以使每次迭代合并更多的元素，也使用了两路并行，将索引值为偶数的元素累积在变量 acc0 中，将索引值为奇数的元素累积在变量 acc1 中。代码详见 code/vec.c 中的 combine6 方法。
