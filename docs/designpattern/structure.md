...
## 4.8 <span id="4.8">结构型模式小结</span>

结构型模式之间比较相似，尤其是它们的参与者和协作者之间的相似性。这可能是因为结构型模式依赖于同一个很小的语言机制集合构造代码和对象：单继承和多重继承机制用于基于类的模式，而对象组合模式用于对象式模式。

**但是这些相似性掩盖了这些模式的不同意图**


### 4.8.1 <span id="4.8.1">Adapter与Bridge</span>

Adapter与Bridge模式具有一些共同的特征。它们都
- 给以另一个对象提供了一定程度上的间接性，因而有利于系统的灵活性。
- 涉及到从自身以外的一个接口向这个对象转发请求。

不同之处在于它们的用途：
- Adapter模式主要解决两个已有接口的不匹配问题，
  - 它不考虑接口是怎样实现的，也不考虑它们各自可能会如何演化。
  - 它不需要对两个独立涉及的类中的任何一个进行重新设计，就能是它们协同工作。
- Bridge模式则对抽象接口与它的实现部分进行桥接，
  - 虽然它允许修改它的实现类，它仍然为用户提供一个稳定的接口
  - 它也会在系统演化时适应新的实现

基于上述的不同点，Adapter与Bridge模式通常用于软件生命周期的不同阶段：
- 事后实施。当发现两个不兼容的类必须同时工作时，就有必要使用Adapter模式
- 事前实施。相反Bridge模式的使用者必须事先知道一个抽象类将有多个实现部分，并且抽象和实现部分独立演化。

Facade定义一个新的接口，Adapter则复用一个原有的接口。


### 4.8.2 <span id="4.8.2">Composite、Decorator与Proxy</span>

Composite与Decorator模式具有类似的类图，这说明它们
- 都基于递归组合来组织可变对象的数目。

但其相似点仅止于递归组合。

Composite、Decorator模式的不同点：
- Decorator模式旨在能够不需要生成子类，即可给对象添加职责。避免静态实现所有的功能组合，导致子类急剧增加。
- Composite模式旨在构造类，使多个相关的对象能够以统一的方式处理。它重点不在于修饰，而在于表示。

Composite、Decorator模式经常协同使用。


Decorator与Proxy模式，两种模式都
- 描述了怎样为对象提供一定程度的间接引用，
- 保留了指向另一个对象的指针/引用，向这个对象发送请求
- 构成一个对象，并为用户提供一致的接口

然而，Decorator与Proxy模式具有不同的设计目的：
- Proxy不能动态的添加或分离性质，也不是为递归组合而设计
- Proxy的目的是，当直接访问一个实体不方便或不符合需求时，为实体提供一个替代者。
- Proxy模式中，实体定义了关键功能，而Proxy提供对实体的访问， Decorator模式中，组件仅提供部分功能，而一个或多个Decorator负责完成其他功能
- Decorator模式适用于编译时不能确定对象的全部功能的情况；Proxy模式中，这种关系可以静态的表达
