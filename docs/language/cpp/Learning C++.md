...
#### 替换文本宏 Replacing text macros
预处理器支持文本宏替换。还支持类似函数的文本宏替换。


##### `#` 和 `##` 运算符
在类似函数的宏中，替换列表中标识符前的 # 运算符通过参数替换运行标识符，并将结果括在引号中，从而有效地创建字符串文字。此外，预处理器会添加反斜杠以转义嵌入的字符串文本周围的引号（如果有），并根据需要将字符串中的反斜杠加倍。删除所有前导和尾随空格，文本中间的任何空格序列（但不在嵌入的字符串文本内）都折叠为单个空格。此操作称为“字符串化”。如果字符串化的结果不是有效的字符串文本，则行为未定义。


替换列表中任意两个连续标识符之间的 `##` 运算符对两个标识符（首先不是宏展开的）运行参数替换，然后连接结果。此操作称为“串联”或“令牌粘贴”。只能粘贴一起构成有效令牌的标记：构成较长标识符的标识符、构成数字的数字或构成 .注释不能通过粘贴创建，因为在考虑宏替换之前会从文本中删除注释。如果串联的结果不是有效的标记，则行为未定义。+=+=/*
