...
观察此树，如果把初始向量 a 中的元素按其在叶中出现的次序排列，称为 **位逆序置换**。就可以对 `recursive_fft` 的执行进行追踪，不过是自底向上而不是自顶向下。首先成对取出元素，利用一次蝴蝶操作计算出，每对的 DFT，然后用其 DFT 取代这对元素。这样向量中包含了 n/2 个二元素的 DFT。下一步，按对取出这 n/2 个 DFT，通过两次蝴蝶操作计算出具有四个元素向量的 DFT，并用一个具有四个元素的 DFT 取代对应的两个二元素的 DFT。于是向量中包含 n/4 个 四元素的 DFT。继续这一过程，直至向量包含两个具有 n/2 个元素的 DFT。
...
注意到最后一行的 $\zeta$ 的指数部分 $brv(\cdot)$ 正是 FFT 介绍过的位逆序置换，其形式化表述如下：
$$
brv_k(m) = brv_k(b_{k-1}\cdots b_1b_0) := (b_0b_1\cdots b_{k-1}), 0\leq m \leq 2^k-1
$$
上式中，m 是能以 k 位二进制比特串表示的正整数，高为不足则补 0。在 Kyber 中，brv 取 7位比特进行逆序置换。


假设根节点处的初始元素为多项式环 
$$
s(X) = a_0 + a_1 + \cdots + a_{n-2 } + a_{n-1} (\bmod X^n - \zeta^{n/2})
$$
则同构映射后，左子结点对应的分量为:
$$
s_0(X) = s(X) (\bmod X^{n/2} - \zeta^{n/4})
$$
右子结点对应的分量为:
$$
s_1(X) = s(X) (\bmod X^{n/2} + \zeta^{n/4})
$$
中国剩余定理表明 $s(X) \mapsto (s_0(X), s_1(X))$ 的映射是同构映射。对左子节点而言，由于在 $(\bmod X^{n/2} - \zeta^{n/4})$ 的意义下，$X^{n/2} - \zeta^{n/4} \equiv (\bmod X^{n/2} + \zeta^{n/4})$。得到 $X^{n/2} = \zeta^{n/4}$；因此原多项式 $s(X)$ 中任何高于 $d/2$ 阶的 X 项，都能以 $\zeta^{n/4}$ 代替 $X^{n/2}$：
$$
\begin{aligned}
...
生成 $z~B_\eta$ 满足 均值为 0 ，方差为 $\eta/2$ 的分布:
$$
P(z=k) = \frac{\binom{2\eta}{\eta+k}}{2^{2\eta}}, k \in [-\eta, \eta]
$$

对于 $\eta = 2$, 各值的概率为 :
- $ p(z=0) = \frac{6}{16}$
- $ p(z=-1) = p(z=1) = \frac{4}{16}$
- $ p(z=-2) = p(z=2) = \frac{1}{16}$

kyber 的私钥及噪音支持CBD分布，而 Dilithium 的私钥和噪音的采用平均分布。
...
```
手工计算一下 $[-2,-1,0,1,2]$ 各取值的概率，从随机数中取奇偶位，并相加。若每位为 0, 1 的概率各为 $50%$。 则 $a, b$ 的值为奇偶位相加 $0+0=0, 1+0=1, 0+1 = 1, 1+1=2$, 因此取值分别为 $0, 1, 2$ 其概率为 $\frac{1}{4}, \frac{1}{2}, \frac{1}{4}$。 

最后 a - b 取值 $[-2, -1, 0, 1, 2]$ 的概率分别为：
- $-2 = 0 - 2$, 概率为 $\frac{1}{4} * \frac{1}{4} = \frac{1}{16}$ 。
- $-1 = 0 - 1 = 1-2$ 概率为 $\frac{1}{4} * \frac{1}{2} + \frac{1}{2} * \frac{1}{4} = \frac{4}{16}$ 。
- $0 = 0 - 0 = 1-1 = 2-2$ 概率为 $\frac{1}{4} * \frac{1}{4} + \frac{1}{2} * \frac{1}{2} * \frac{1}{4} * \frac{1}{4} = \frac{6}{16}$ 。
- $1 = 2 - 1 = 1-0$ 概率为 $\frac{1}{4} * \frac{1}{2} + \frac{1}{2} * \frac{1}{4} = \frac{4}{16}$ 。
- $2 = 2 - 0$, 概率为 $\frac{1}{4} * \frac{1}{4} = \frac{1}{16}$ 。
