...
7x^3 + 2x^2 - x + 3 \\
4x^3 - 2x^2 + 5x - 1
\end{bmatrix} = \begin{bmatrix} [3,-1,2,7] \\ [-1,5,-2,4] \end{bmatrix}
$$
...
### 计算步骤
$$
\mathbf{t} = \mathbf{A} \cdot \mathbf{s} = \begin{bmatrix}
A_{11}s_1 + A_{12}s_2 \\
A_{21}s_1 + A_{22}s_2
\end{bmatrix}
$$
...
&= \textcolor{blue}{77x^6 + 22x^5 - 11x^4 + 33x^3} \\
&\quad + \textcolor{green}{42x^5 + 12x^4 - 6x^3 + 18x^2} \\
&\quad + \textcolor{red}{-15x^4 - 4x^3 + x^2 - 3x} \\
&\quad + \textcolor{purple}{56x^3 + 16x^2 - 8x + 24} \\
&+ \textcolor{orange}{20x^6 - 10x^5 + 25x^4 - 5x^3} \\
&\quad + \textcolor{brown}{52x^5 - 26x^4 + 65x^3 - 13x^2} \\
&\quad + \textcolor{cyan}{36x^4 - 18x^3 + 45x^2 - 9x} \\
&\quad + \textcolor{magenta}{8x^3 - 4x^2 + 10x - 2} \\
&\equiv 97x^6 + 106x^5 + 21x^4 + 129x^3 + 63x^2-10x+22\\
&\equiv (-97)x^2 + (-106)x + (-21) + 129x^3 + 63x^2-10x+22\\
&\equiv 129x^3 -34x^2 - 116x + 1\\
&\equiv 10x^3 + 0x^2 + 3x + 1 \quad (\text{mod } x^4+1, \text{mod }17)
\end{aligned}
$$
...
| 分量 | 无噪声结果 | 含噪声结果 | 变化量 |
|------|------------|------------|--------|
| t₁   | $11x^3 + 11x^2 + 6x + 3$ | $11x^3 + 12x^2 + 5x + 3$ | $+x^2 -x$ |
| t₂   | $8x^3 + 14x^2 + 11x + 3$ | $8x^3 + 14x^2 + 11x + 3$ | 0 |
...
7x^3 + 4x^2 + 12x + 1 & 14x^3 + 3x^2 + 10x + 16 
\end{bmatrix}, \quad
\mathbf{t} = \begin{bmatrix} 11x^3 + 11x^2 + 6x + 3 \\ 8x^3 + 14x^2 + 11x + 3 \end{bmatrix}
$$
...
$$
\begin{cases}
(11x^3+6x^2+15x+8)s_0 + (5x^3+13x^2+9x+2)s_1 = 11x^3+11x^2+6x+3 \quad (1) \\
...
$$
\begin{aligned}
x^3: & \quad 11a_3 + 5b_3 = 11 \\
x^2: & \quad 6a_3 + 13b_3 + 11a_2 + 5b_2 = 11 \\
x^1: & \quad 15a_3 + 9b_3 + 6a_2 + 13b_2 + 11a_1 + 5b_1 = 6 \\
x^0: & \quad 8a_3 + 2b_3 + 15a_2 + 9b_2 + 6a_1 + 13b_1 + 11a_0 + 5b_0 = 3
\end{aligned}
$$
...
$$
\begin{aligned}
x^3: & \quad 7a_3 + 14b_3 = 8 \\
x^2: & \quad 4a_3 + 3b_3 + 7a_2 + 14b_2 = 14 \\
x^1: & \quad 12a_3 + 10b_3 + 4a_2 + 3b_2 + 7a_1 + 14b_1 = 11 \\
x^0: & \quad 1a_3 + 16b_3 + 12a_2 + 10b_2 + 4a_1 + 3b_1 + 7a_0 + 14b_0 = 3
\end{aligned}
$$
...
从方程(2)的$x^3$项：
$$
7a_3 + 14b_3 \equiv 8 \quad (\text{mod }17)
$$
两边乘以$7^{-1} \equiv 5$：
$$
a_3 + 2b_3 \equiv 40 \equiv 6 \quad (\text{mod }17)
$$
得：
$$
a_3 \equiv 6 - 2b_3 \quad (3)
$$

代入方程(1)的$x^3$项：
$$
11(6-2b_3) + 5b_3 \equiv 11 \\
66 - 22b_3 + 5b_3 \equiv 11 \\
-17b_3 \equiv -55 \\
b_3 \equiv 55 \times (-17)^{-1} \equiv 55 \times 16 \equiv 880 \equiv 13 \quad (\text{mod }17)
$$

回代(3)式：
$$
a_3 \equiv 6 - 2 \times 13 \equiv -20 \equiv 14 \quad (\text{mod }17)
$$

### 2. 解$x^2$系数
已知$a_3=14,b_3=13$，代入方程(2)的$x^2$项：
$$
4 \times 14 + 3 \times 13 + 7a_2 + 14b_2 \equiv 14 \\
56 + 39 + 7a_2 + 14b_2 \equiv 14 \\
7a_2 + 14b_2 \equiv 14 - 95 \equiv -81 \equiv 4 \quad (\text{mod }17)
$$
两边乘以$7^{-1} \equiv 5$：
$$
a_2 + 2b_2 \equiv 20 \equiv 3 \quad (4)
$$

从方程(1)的$x^2$项：
$$
6 \times 14 + 13 \times 13 + 11a_2 + 5b_2 \equiv 11 \\
84 + 169 + 11a_2 + 5b_2 \equiv 11 \\
11a_2 + 5b_2 \equiv 11 - 253 \equiv -242 \equiv 8 \quad (\text{mod }17)
$$
...
解得：
$$
a_2 \equiv 16, \quad b_2 \equiv 10 \quad (\text{mod }17)
$$
...
$$
\begin{aligned}
a_1 &\equiv 2, \quad b_1 \equiv 15 \quad (\text{mod }17) \\
a_0 &\equiv 5, \quad b_0 \equiv 9 \quad (\text{mod }17)
\end{aligned}
$$
...
13x^3 + 10x^2 + 15x + 9 
\end{bmatrix}
= \begin{bmatrix} [5,2,16,14] \\ [9,15,10,13] \end{bmatrix}
}
$$
...
```python
# 验证第一分量
t1_calc = (11x³+6x²+15x+8)(14x³+16x²+2x+5) + (5x³+13x²+9x+2)(13x³+10x²+15x+9)
         ≡ 11x³ + 11x² + 6x + 3 (mod x⁴+1, mod 17)
```
与给定t₁一致
