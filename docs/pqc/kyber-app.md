...
<img src="https://img-blog.csdnimg.cn/60f1028dbb5941cd80e9c3016f6b9698.png" alt="单位复数根">
...
举例说明，n = 256,
初始计算需要计算 $\omega_{256}^0, \omega_{256}^1, \omega_{256}^2, \omega_{256}^3, \cdots, \omega_{256}^{255} $ 共计 256 个单位根
第 1 次递归计算需要计算 $\omega_{128}^0, \omega_{128}^1, \omega_{128}^2, \omega_{128}^3, \cdots, \omega_{128}^{127} $ 共计 128 个单位根，$A^{[0]}(x)$ 和 $A^{[1]}(x) 2 个式子；$例如 $(\omega_{256}^1)^2 = (\omega_{256}^{129})^2 = \omega_{128}^1$， 
第 2 次递归计算需要计算 $\omega_{64}^0, \omega_{64}^1, \omega_{64}^2, \omega_{64}^3, \cdots, \omega_{64}^{63} $ 共计 64 个单位根，4 个式子
第 3 次递归计算需要计算 $\omega_{32}^0, \omega_{32}^1, \omega_{32}^2, \omega_{32}^3, \cdots, \omega_{32}^{31} $ 共计 32 个单位根，8 个式子
第 4 次递归计算需要计算 $\omega_{16}^0, \omega_{16}^1, \omega_{16}^2, \omega_{16}^3, \cdots, \omega_{16}^{15} $ 共计 16 个单位根，16 个式子
第 5 次递归计算需要计算 $\omega_{8}^0, \omega_{8}^1, \omega_{8}^2, \omega_{8}^3, \cdots, \omega_{8}^{7} $ 共计 8 个单位根，32 个式子
第 6 次递归计算需要计算 $\omega_{4}^0, \omega_{32}^1, \omega_{32}^2, \omega_{32}^3 $ 共计 4 个单位根，64 个式子
第 7 次递归计算需要计算 $\omega_{2}^0, \omega_{2}^1 $ 共计 2 个单位根，128个式子
第 8 次递归计算需要计算 $\omega_{1}^0$ 共计 1 个单位根，256 个式子，

因此，仅需要 $\log 256 = 8$ 次递归，时间复杂度为 
