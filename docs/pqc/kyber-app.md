...
# 6. 环与多项式：Kyber的数学舞台
Kyber 的核心操作是多项式乘法。一个看似简单的 256 次多项式乘法，若用传统方法计算，需要进行 256×256=65,536 次系数乘加——这相当于在手机上执行一次 TLS 握手就需要消耗整个屏幕刷新周期（16ms）的计算资源。这种计算开销对实时通信系统是不可接受的。

工程师们首先尝试了经典优化算法。Karatsuba 算法可以将计算量减少到约3,000次操作，但会引发系数膨胀，导致后续模运算出错；Toom-Cook 算法虽然理论上更快，但实现复杂度使其在实际硬件中反而更慢。最直接的查表法需要预存 $256^3=16,777,216$ 个系数，这远超手机处理器的 L2 缓存容量。

**转折点**出现在快速傅里叶变换（FFT）的应用上。FFT通过巧妙的分治策略，将多项式转换为点值表示：选取特定的"单位根"作为采样点，利用对称性将计算复杂度从 $O(n^2)$ 降至 $O(n\log n)$。对于 $n=256$ 的情况，仅需 2,048 次操作即可完成乘法——这是革命性的突破。

但FFT存在致命缺陷：它依赖复数运算。在256次蝴蝶变换的迭代过程中，浮点误差会累积到 $10^{-14}$ 量级。这个看似微小的误差会导致模 3329 运算时出现致命错误——假设理论结果应为0，实际可能得到 0.00000000000012 ，取整后变成 1，最终导致解密失败。在密码学中，即使是十亿分之一的误差率也是不可接受的。

正是这种"速度与精度不可兼得"的困境，促使密码学家寻找新的数学工具。我们需要一种既能保持FFT的快速特性，又能实现精确整数运算的方法——这个目标将引领我们探索数论变换（NTT）的奥秘。


## 6.1 快速傅里叶变换（FFT）的启示

多项式乘法的困境，假设我们需要计算两个256次多项式：
$$
A(x) = a_0 + a_1x + ... + a_{255}x^{255}  \\
B(x) = b_0 + b_1x + ... + b_{255}x^{255}
$$

传统方法需要：65,536次系数相乘（每个a_i × b_j）超过50,000次加法,这相当于用中世纪的方法建造摩天大楼——理论上可行，实践中荒谬。

FFT的魔法：从系数到点值
核心思想
"与其硬算乘积，不如先转换成更容易操作的形式"：

点值表示定理：n次多项式可由n+1个点唯一确定
策略：

将A(x)和B(x)在特定点集{x₀,x₁,...,x₂ₙ}上求值

对应点值相乘得到C(xₖ)=A(xₖ)×B(xₖ)
...
复杂度分析: 乘法次数：从O(n²)降至O(n log n) 实际案例：n=256 → 仅需2,048次操作


FFT的密码学缺陷,浮点误差的雪崩效应 每级蝴蝶变换引入约10⁻¹⁶误差 经过log₂256=8级迭代后, 这会导致模3329运算时出现±1的致命偏差
