...
$$
\begin{aligned}
c &= (14x^3+12x^2+12x)(-x^3+x^2) \\
&\quad + (2x^3+6x)(x^3+x^2-1) + (-x^3-x^2) + (9x^3 + 9x + 9)\\
&= -14x^6 - 12x^5 - 12x^4\\
&\quad + 14x^5 + 12x^4 + 12x^3\\
&\quad + 2x^6 + 2x^5 - 2x^3\\
&\quad + 6x^4 + 6x^3 - 6x\\
&\quad + (-x^3-x^2) + (9x^3 + 9x + 9)\\
&\equiv (-14+2)x^6 + (-12+14+2)x^5 + (-12+12+6)x^4\\
&\quad +(12-2+6)x^3 -6x + (-x^3-x^2) + (9x^3 + 9x + 9)\\
&\equiv -12x^6 + 4x^5 + 6x^4 + 16x^3 - 6x + (-x^3-x^2) + (9x^3 + 9x + 9) (\bmod(x^4+1)) \\
&\equiv 12x^2 - 4x - 6 + 16x^3 -6x + (-x^3-x^2) + (9x^3 + 9x + 9) \\  
&\equiv 16x^3 + 12x^2 - 10x - 6 + (-x^3-x^2) + (9x^3 + 9x + 9) \\
&\equiv 24x^3 + 11x^2 - 1x + 3 (\bmod(17)) \\
&\equiv 7x^3 + 11x^2 + 16x + 3 \\
\end{aligned}
$$
...
&\quad - 2x^6 - 7x^5 - 15x^4 - 5x^3 \\
&\quad + 2x^4 + 7x^3 + 15x^2 + 5x \\
&\equiv (-2)x^6 + (13-7)x^5 + (10-13-15+2)x^4  \\
&\quad + (7-10+13-5+7)x^3 + (9-7+10+15)x^2 + (-9+7+5)x + 9\\
&\equiv -2x^6 + 6x^5 - 16x^4 + 12x^3 + 27x^2 +3x -9 (\bmod(x^4+1))  \\
&\equiv 2x^2 - 6x + 16 + 12x^3 + 27x^2 +3x + 9 \\
&\equiv 12x^3 + 29x^2 - 3x + 25  (\bmod(17))\\
&\equiv 12x^3 + 12x^2 + 14x + 8  (\bmod(17))\\
\end{aligned}
$$

#### 2.1.4.2. 计算 $m' = c - \mathbf{u}^T \mathbf{s}$
$$
\begin{aligned}
m' &= 7x^3 + 11x^2 + 16x + 3 - (12x^3 + 12x^2 + 14x + 8) \\
&\equiv -5x^3 - 1x^2 + 2x - 5  (\bmod(17))\\
&\equiv 8x^3 + 16^2 + 2x + 8
\end{aligned}
$$
...
# Mini-Kyber 加解密完整流程（n=4, q=17, k=2, η=1）
...
$$
\mathbf{A} = \begin{bmatrix} 
[8,15,6,11]^T & [2,9,13,5]^T \\ 
[1,12,4,7]^T & [16,10,3,14]^T 
\end{bmatrix}
$$
...
## 2. 加密过程
### 步骤1：明文编码
采用 **round(q/2) = 9** 作为缩放因子（因 q=17，17/2=8.5→四舍五入为9）：
$$ \mathbf{m}_{enc} = [9,9,0,9] $$

### 步骤2：生成随机向量和噪声
- 随机向量：$\mathbf{r} = [1,0,-1,1]$ （系数 ∈ {-1,0,1}）
- 加密噪声：
  - $\mathbf{e}_1 = \begin{bmatrix} [0,1,-1,0]^T \\ [0,0,0,0]^T \end{bmatrix}$
  - $e_2 = 1$

### 步骤3：计算密文 $\mathbf{c}_1 = \mathbf{A}^T \mathbf{r} + \mathbf{e}_1$
#### $\mathbf{c}_{11} = \mathbf{A}_{11} \cdot \mathbf{r} + \mathbf{e}_{11}$
$$
\begin{aligned}
&(11x^3+6x^2+15x+8)(x^3-x+1) + (x^2-x) \\
&= [ -11x^2 + 6x^3 -15 + 8x^3 ] + [x^2 - x] \\
&= [ -10x^2 + 14x^3 - x -15 ] \\
&\equiv [2, 16, 16, 14] \quad (\text{mod }17)
\end{aligned}
$$

#### $\mathbf{c}_{12} = \mathbf{A}_{12} \cdot \mathbf{r} + \mathbf{e}_{12}$
$$
\begin{aligned}
&(5x^3+13x^2+9x+2)(x^3-x+1) + 0 \\
&= [ -5x^2 + 13x^3 -9 + 2x^3 ] \\
&= [ -5x^2 + 15x^3 -9 ] \\
&\equiv [8, 12, 12, 15] \quad (\text{mod }17)
\end{aligned}
$$

#### $\mathbf{c}_{21} = \mathbf{A}_{21} \cdot \mathbf{r} + \mathbf{e}_{21}$
$$
\begin{aligned}
&(7x^3+4x^2+12x+1)(x^3-x+1) + 0 \\
&= [ -7x^2 + 4x^3 -12 + x^3 ] \\
&= [ -7x^2 + 5x^3 -12 ] \\
&\equiv [5, 10, 10, 5] \quad (\text{mod }17)
\end{aligned}
$$

#### $\mathbf{c}_{22} = \mathbf{A}_{22} \cdot \mathbf{r} + \mathbf{e}_{22}$
$$
\begin{aligned}
&(14x^3+3x^2+10x+16)(x^3-x+1) + 0 \\
&= [ -14x^2 + 3x^3 -10 + 16x^3 ] \\
&= [ -14x^2 + 19x^3 -10 ] \\
&\equiv [9, 3, 3, 2] \quad (\text{mod }17)
\end{aligned}
$$

### 步骤4：计算 $\mathbf{c}_2 = \mathbf{t}^T \mathbf{r} + e_2 + \mathbf{m}_{enc}$
$$
\begin{aligned}
\mathbf{c}_2 &= (14x^3+12x^2+12x)(x^3-x+1) + 1 + [9,9,0,9] \\
&= [ -14x^2 + 12x^3 -12x ] + [10,9,0,9] \\
&= [ -14x^2 + 12x^3 -12x + 10 ] \\
&\equiv [10, 5, 3, 12] \quad (\text{mod }17)
\end{aligned}
$$

### 最终密文
$$
\mathbf{c}_1 = \begin{bmatrix} [2,16,16,14] \\ [8,12,12,15] \end{bmatrix}, \quad
\mathbf{c}_2 = [10,5,3,12]
$$

## 3. 解密过程
### 步骤1：计算 $\mathbf{u} = \mathbf{c}_1 \cdot \mathbf{s}$
$$
\begin{aligned}
u &= [2,16,16,14] \cdot [1,-1,1,0] + [8,12,12,15] \cdot [0,1,0,-1] \\
&= [ -14, 16, -14, 2 ] + [ -15, 12, -15, 8 ] \\
&= [ -29, 28, -29, 10 ] \\
&\equiv [5, 11, 5, 10] \quad (\text{mod }17)
\end{aligned}
$$

### 步骤2：恢复明文
$$
\mathbf{m}_{rec} = \mathbf{c}_2 - \mathbf{u} = [10-5, 5-11, 3-5, 12-10] = [5, -6, -2, 2]
$$

### 步骤3：解码（缩放因子9）
1. 计算 $9^{-1} \equiv 2 \pmod{17}$
2. 乘逆元：
   $$ [5×2, -6×2, -2×2, 2×2] = [10, -12, -4, 4] \equiv [10,5,13,4] \pmod{17} $$
3. 四舍五入：
   $$ \left[\mathrm{round}(\frac{10}{9}), \mathrm{round}(\frac{5}{9}), \mathrm{round}(\frac{13}{9}), \mathrm{round}(\frac{4}{9})\right] = [1,1,1,0] $$
