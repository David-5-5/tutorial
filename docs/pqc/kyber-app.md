...
性质： 
- n 次本原单位根的数量为 $\phi(n)$ (欧拉函数，表示小于 n 且与 n 互质的正整数个数)
- 例如 n = 4 时，$\phi(4) = 2$，本原单位根为 i 和 -i； n = 3 时，$\phi(3) = 2$，本原单位根为 $e^{2\pi i/3}$ 和 $e^{4\pi i/3}$


**分圆多项式**（Cyclotomic Polynomial）分圆多项式是数论和代数中一类重要的多项式，与单位根密切相关，在数论、代数数论、群论等领域有广泛应用。以下从定义、性质、构造方法及应用等方面详细介绍。

定义： 分圆多项式 $\Phi_n(x)$ 是一个首一多项式，其根为所有 本原 n 次单位根
