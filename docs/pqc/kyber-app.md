...
若 $f$ 和 $g$ 在相同点集求值，乘积 $h$ 的点值可直接计算：$$ h(x_i) = f(x_i) \cdot g(x_i) $$ 仅需 $O(n)$ 时间。如何高效的将多项式环的系数表示转换为点值表示，再完成多项式乘法，就是提升多项式环的关键。


### 6.1.2 快速傅里叶变换 FFT
多项式环中从系数表示法转换为点值表示法的主要方法可分为直接计算法和快速变换法两类，其中直接计算法包括霍纳法则和拉格朗日插值法，但是直接计算的时间复杂度为 $O(n^2)$，这样多项式环乘法的总体的时间复杂度并未减少。

快速傅里叶变换通过分治(二分法)的思想，将多项式环的系数和点值表示的转换时间降低到 $O(n\log n)$，其核心思想：
多项式拆分，将 $f(x)$ 按奇偶次项分解:
$$
...
$$
\begin{aligned} 
f_{even}(x^2) &= a_0 + a_2x + \cdots + a_{2k}x^{2k} + \cdots \\
f_{odd}(x^2) &= a_1 + a_3x + \cdots + a_{2k+1}x^{2k} + \cdots
\end{aligned} 
$$
如果通过选择特殊的点 $x_0, x_1, x_2, \cdots, x_n$，将递归进行执行下去 
