...
|方法|Head|Worker|作用|
|------|------|--------|------|
|`__init__()`|是|是|初始化，调用下面两个方法|
|`start_head_processes()`|是|否|GCS、Monitor、API Server|
|`start_ray_processes()`|是|是|Raylet、Agent、Log Monitor|
|`dead_processes()`|是|是|获取已死亡的子进程|
|`kill_all_processes()`|是|是|杀死所有子进程|
|`check_version_info()`|否|是|Worker 版本校验|
...
|启动方式|复杂度|推荐度|适用场景|
|---------|--------|--------|---------|
|**`ray start --head`**|可选|强烈推荐|日常开发、生产环境|
|**Python API (`services.py`)**|次要|仅限特殊场景|调试、定制化启动、测试|
|**纯命令行手动**|不推荐|不推荐|仅用于学习底层机制、gdb 调试|
