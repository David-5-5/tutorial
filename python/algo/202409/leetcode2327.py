...
class Solution:
    ...
    def peopleAwareOfSecret2(self, n: int, delay: int, forget: int) -> int:
        ...
        for i in range(2, n+1):
            ...
            # for j in range(max(i-forget+1,0), max(i-delay,0)+1):
            #     secret[i][1] += secret[j][1]
            secret[i][1] = presum[max(i-delay,0)+1]-presum[max(i-forget+1,0)]      
            presum.append((presum[-1] + secret[i][1])%mod)
            # 包含新增人数
            secret[i][0] = (secret[i][0] + secret[i][1]) % mod
        return secret[-1][0]
