...
class Solution:
    ...
    def camelLexical(self, s:str):
        ...
        for i in range(1, len(s)):
            if s[i] in CAPITALS:
                if s[begin] in CAPITALS :result.append(s[begin:i])
                begin = i
...
if __name__ == "__main__":
    sol = Solution()
    queries, pattern = ["uAxaqlzahfialcezsLfj","cAqlzyahaslccezssLfj","AqlezahjarflcezshLfj","AqlzofahaplcejuzsLfj","tAqlzahavslcezsLwzfj","AqlzahalcerrzsLpfonj","AqlzahalceaczdsosLfj","eAqlzbxahalcezelsLfj"], "AqlzahalcezsLfj"
