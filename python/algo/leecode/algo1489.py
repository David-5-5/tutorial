...
class Solution:
    # Use Kruskal algorithm of mst
    # The correct solution commit as 2024.06.09
    def findCriticalAndPseudoCriticalEdges(self, n: int, edges) :
        critical, pseudo = [], []
        ne = len(edges)

        sortedges =  sorted(enumerate(edges), key = lambda p:p[1][2])
        minw = 0
        def kruskal(s_edges, exclude:int, include:int):
            ...
            for inx, edge in s_edges:
                if inx == exclude:continue
        ...
        _, minw = kruskal(sortedges, -1, -1)
        for i in range(ne):
            cnt, weight = kruskal(sortedges, i, -1)
            if cnt < n-1 or weight > minw:
                critical.append(i)
        
        for i in range(ne):
            if i in critical:continue
            cnt, weight = kruskal(sortedges, -1, i)
            if cnt == n-1 or weight == minw:
                pseudo.append(i)
