            ...
            while pr:
                ...
                for e in graph[u]:
                    if not visited[e[0]]:
                        w = e[1]
                        if w == -1:
                            w = spec[(min(u,e[0]),max(u,e[0]))] = max(delta - dist[u] + d0[e[0]],1)
                        dist[e[0]] = min(dist[e[0]], w+dist[u])
                        if not edges[min(e[0],u)*n+max(e[0],u)]:
                            heapq.heappush(pr, (dist[e[0]], e[0]))
                            edges[min(e[0],u)*n+max(e[0],u)] = True
